"""
skin_train_enhance.py
=====================
SkinAnalyzerEnhanced 학습 스크립트 — bottleneck FrequencyGate + smooth loss 버전

skin_train_smooth.py 대비 변경사항:
  - skin_net_enhance 사용 (bottleneck enc4에도 FrequencyGate/HighFreqGate 적용)
  - gate 로깅: enc1~enc4 (freq4 포함) 4단계 모니터링
  - checkpoint_dir / preview_dir → _enhance 분리
...
----
  ...
  # 학습
  python skin_train_enhance.py

  # 이어서 학습
  python skin_train_enhance.py --resume ./checkpoints_enhance/last_checkpoint.pth
"""

# ir_encoder_stable을 ir_encoder로 주입 (skin_net_enhance.py 수정 없이 교체)
import sys
...
CFG = dict(
    patch_dir      = './patches',
    checkpoint_dir = './checkpoints_enhance',   # smooth와 분리
...
# ══════════════════════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description='SkinAnalyzerEnhanced 학습')
    ...
    print(f"Model   : SkinAnalyzerEnhanced  (bottleneck enc4에 FrequencyGate 추가)")
    ...
    for epoch in epoch_bar:
        ...
        # Enhanced: enc1~enc4 gate 모두 모니터링 (freq4 = bottleneck)
        ...
        low_gates = [
            ...
            torch.sigmoid(enc.freq4.low_logit).mean().item(),   # ★ bottleneck
        ]
        ...
        tqdm.write(
            ...
            f"  gate=[{low_gates[0]:.3f},{low_gates[1]:.3f},"
            f"{low_gates[2]:.3f},{low_gates[3]:.3f}]"
